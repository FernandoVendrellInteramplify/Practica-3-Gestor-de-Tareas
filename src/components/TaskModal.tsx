    ...
    return (
        <>
            <button type="button" onClick={() => setOpen(true)} title="Añadir Tarea"
                className="rounded-lg bg-blue-600 px-2 py-2 text-sm font-medium text-zinc-50 hover:bg-blue-800">
                {buttonText}
            </button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 ">
                    <section>
                        ...
                        <Confirm formAction={async (formData) => {await formAction(formData);setOpen(false);}}
                            ...
                            <input type="text" name="titulo" required value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título de la tarea"/>

                            <input type="text" name="descripcion" required placeholder="Descripción de la tarea"/>
